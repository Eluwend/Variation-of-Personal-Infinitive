...
#################
        ...
        #Subject
          ...
          #Object:                    
                    #Brazilian:
                    Obj_BR_BlogPra <- read.csv("Pra-Object/Pra-BR-Blog.csv", header=T, sep=",", dec=".", stringsAsFactors=T, skip=4)
...
#Pronoun
  ...
  #pra
      ...
      SubPPra <- SubPra %>% mutate(Pronoun = str_extract(KWIC, "(?<=pra).*(=?\\s)"))
...
for (row in 1:nrow(CorrectedGenresPra))
{
  ...
  if (url == "jus.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
  else
    if (url == "espada.eti.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
    else
      if (url == "montfort.org.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
      else
        if (url == "ocaminho.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
        else
          if (url == "luteranos.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
          else
            if (url == "direitonet.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
            else
              if (url == "espacojames.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
              else
                if (url == "rainhamaria.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
                else
                  if (url == "peticaopublica.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
                  else
                    if (url == "portalsaofrancisco.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
                    else
                      if (url == "carloscastellobranco.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
                      else
                        if (url == "mec.gov.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
                        else
                          if (url == "uol.com.br") CorrectedGenresPra[row, 'Genre'] <- "Formal"
}
...
OutcomePra <- CorrectedGenresPra  %>% mutate(OutcomePra = ifelse(str_detect(Person_of_Infinitive, "1SG|1PL|2SG|2PL|3PL"), "Personal infinitive", "Impersonal infinitive"))

names(OutcomePra) <- names(Outcome)
OutcomeBoth <- rbind(Outcome, OutcomePra)
...
OutcomeBoth <- OutcomeBoth[!grepl("-", OutcomeBoth$MatrixVerb),]
  ...
  OutcomeBoth$Outcome <- as.factor(OutcomeBoth$Outcome)
  OutcomeBoth$Register <- as.factor(OutcomeBoth$Register)
  OutcomeBoth$Variety <- as.factor(OutcomeBoth$Variety)
  OutcomeBoth$Structure <- as.factor(OutcomeBoth$Structure)
  OutcomeBoth$Pronoun <- as.factor(OutcomeBoth$Pronoun)
  OutcomeBoth$Person_of_Infinitive <- as.factor(OutcomeBoth$Person_of_Infinitive)
  OutcomeBoth$Type <- as.factor(OutcomeBoth$Type)
  ...
  Model <- glm(Outcome ~ Register + Genre + Variety + Pronoun + Type, data=OutcomeBoth, family="binomial")
  ModelR <- glm(Outcome ~           Genre + Variety + Pronoun + Type, data=OutcomeBoth, family="binomial")
  ModelG <- glm(Outcome ~ Register +         Variety + Pronoun + Type, data=OutcomeBoth, family="binomial")
  ModelV <- glm(Outcome ~ Register + Genre +         + Pronoun + Type, data=OutcomeBoth, family="binomial")
  ModelP <- glm(Outcome ~ Register + Genre + Variety +         + Type, data=OutcomeBoth, family="binomial")
  ModelT <- glm(Outcome ~ Register + Genre + Variety + Pronoun, data=OutcomeBoth, family="binomial")
