...
#Pronoun
  ...
  #pra
      ...
      SubPPra <- SubPra %>% mutate(Pronoun = str_extract(KWIC, "(?<=\\spara\\s).*?(?=\\s)"))
...
OutcomeBoth <- rbind(Outcome, OutcomePra) %>% filter(Pronoun!="vo")
...
OutcomeBoth <- OutcomeBoth[!grepl("-", OutcomeBoth$MatrixVerb),]
  ...
  Model <- glm(Outcome ~ Register + Variety + Pronoun, data=OutcomeBoth, family="binomial")
